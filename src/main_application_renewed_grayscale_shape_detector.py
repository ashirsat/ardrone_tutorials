...
class CVGUI(QtGui.QMainWindow):
    ...
    def cvRedrawCallback(self): ## Done
        if self.cvimage is not None:
            ...
            try:
                cvimage = QtGui.QPixmap(QtGui.QImage(self.cvimage, 640, 360, QtGui.QImage.Format_Mono))#Format_RGB888))
            finally:
    ...
    def cvReceiveImage(self, data):
        ...
        try:
            ...
            self.cvimg_med = cv2.medianBlur(gray, 3)
            self.cvimg_bilat = cv2.bilateralFilter(self.cvimg_med, 3, sigmaColor=10, sigmaSpace=5)
            self.thresh = cv2.threshold(self.cvimg_med, 145, 200, cv2.THRESH_BINARY)[1]
            self.thresh = cv2.erode(self.thresh, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)), iterations=4)
            self.thresh = cv2.dilate(self.thresh, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)), iterations=4)
            contours, _ = cv2.findContours(self.thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1)
            # cnt = contours
            shape="undefined"
            Cx = 400
            Cy = 400
            for c in contours:
                ...
                apprx = cv2.approxPolyDP(c, 0.04 * peri, True)
            ...
            # # sub_mask = cv2.morphologyEx(sub_mask, cv2.MORPH_OPEN, self.back_ker)
                ...
                cv2.putText(cvimg, shape, (Cx, Cy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 4)
            self.cvimage = gray
        finally:
            # pass
            self.cvimageLock.release()
