...
class DroneVideoDisplay2(QtGui.QMainWindow):
    StatusMessages = {
        DroneStatus2.Emergency : 'Emergency',
        DroneStatus2.Inited    : 'Initialized',
        DroneStatus2.Landed    : 'Landed',
        DroneStatus2.Flying    : 'Flying',
        DroneStatus2.Hovering  : 'Hovering',
        DroneStatus2.Test      : 'Test (?)',
        DroneStatus2.TakingOff : 'Taking Off',
        DroneStatus2.GotoHover : 'Going to Hover Mode',
        DroneStatus2.Landing   : 'Landing',
        DroneStatus2.Looping   : 'Looping (?)'
        }

    DisconnectedMessage2 = 'Disconnected'
    UnknownMessage2 = 'Unknown Status'

    def __init__(self):
        # Construct the parent class
        super(DroneVideoDisplay2, self).__init__()
        ...
        self.subNavdata = rospy.Subscriber('/Alpha2/ardrone/navdata',Navdata,self.ReceiveNavdata)

        # Subscribe to the drone's video feed, calling self.ReceiveImage when a new frame is received
        self.subVideo   = rospy.Subscriber('/Alpha2/ardrone/image_raw',Image,self.ReceiveImage)
    ...
    def RedrawCallback(self):
        ...
        self.statusBar().showMessage(self.statusMessage if self.connected else self.DisconnectedMessage2)
    ...
    def ReceiveNavdata(self,navdata):
        ...
        msg = self.StatusMessages[navdata.state] if navdata.state in self.StatusMessages else self.UnknownMessage2
        self.statusMessage = '{} (Battery: {}%)'.format(msg,int(navdata.batteryPercent))

        # self.tagLock2.acquire()
        # try:
        #     if navdata.tags_count > 0:
        #         self.tags2 = [(navdata.tags_xc[i],navdata.tags_yc[i],navdata.tags_distance[i]) for i in range(0,navdata.tags_count)]
        #     else:
        #         self.tags2 = []
        # finally:
        #     self.tagLock2.release()
...
if __name__=='__main__':
    import sys
    rospy.init_node('ardrone_video_display2')
    app = QtGui.QApplication(sys.argv)
    display = DroneVideoDisplay2()
    display.show()
    status = app.exec_()
