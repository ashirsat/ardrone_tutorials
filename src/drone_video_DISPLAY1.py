...
class DroneVideoDisplay1(QtGui.QMainWindow):
    StatusMessages = {
        DroneStatus1.Emergency : 'Emergency',
        DroneStatus1.Inited    : 'Initialized',
        DroneStatus1.Landed    : 'Landed',
        DroneStatus1.Flying    : 'Flying',
        DroneStatus1.Hovering  : 'Hovering',
        DroneStatus1.Test      : 'Test (?)',
        DroneStatus1.TakingOff : 'Taking Off',
        DroneStatus1.GotoHover : 'Going to Hover Mode',
        DroneStatus1.Landing   : 'Landing',
        DroneStatus1.Looping   : 'Looping (?)'
        }

    DisconnectedMessage1 = 'Disconnected'
    UnknownMessage1 = 'Unknown Status'

    def __init__(self):
        # Construct the parent class
        super(DroneVideoDisplay1, self).__init__()
        ...
        self.subNavdata = rospy.Subscriber('/Alpha1/ardrone/navdata',Navdata,self.ReceiveNavdata)

        # Subscribe to the drone's video feed, calling self.ReceiveImage when a new frame is received
        self.subVideo   = rospy.Subscriber('/Alpha1/ardrone/image_raw',Image,self.ReceiveImage)
    ...
    def RedrawCallback(self):
        ...
        self.statusBar().showMessage(self.statusMessage if self.connected else self.DisconnectedMessage1)
    ...
    def ReceiveNavdata(self,navdata):
        ...
        msg = self.StatusMessages[navdata.state] if navdata.state in self.StatusMessages else self.UnknownMessage1
        self.statusMessage = '{} (Battery: {}%)'.format(msg,int(navdata.batteryPercent))

        # self.tagLock1.acquire()
        # try:
        #     if navdata.tags_count > 0:
        #         self.tags1 = [(navdata.tags_xc[i],navdata.tags_yc[i],navdata.tags_distance[i]) for i in range(0,navdata.tags_count)]
        #     else:
        #         self.tags1 = []
        # finally:
        #     self.tagLock1.release()
...
if __name__=='__main__':
    import sys
    rospy.init_node('ardrone_video_display1')
    app = QtGui.QApplication(sys.argv)
    display = DroneVideoDisplay1()
    display.show()
    status = app.exec_()
